...
const svgFiles = fs.readdirSync(iconsSourceDir).filter(file => file.toLowerCase().endsWith(".svg")).map(file => path.join(iconsSourceDir, file));
...
(async function processIcons() {
  try {
    for (const svgPath of svgFiles) {
      ...
      execSync(vectorDrawableCommand, {
        stdio: "inherit"
      });
