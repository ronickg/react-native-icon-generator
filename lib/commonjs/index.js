...
const iosXcassetsDir = _path.default.join(iosOutputDir, "Icons.xcassets");

// Ensure output directories exist
try {
  _fs.default.mkdirSync(tempSvgsDir, {
    recursive: true
  });
  _fs.default.mkdirSync(androidOutputDir, {
    recursive: true
  });
  _fs.default.mkdirSync(iosOutputDir, {
    recursive: true
  });
  _fs.default.mkdirSync(iosXcassetsDir, {
    recursive: true
  });
  ...
  _fs.default.writeFileSync(_path.default.join(iosXcassetsDir, "Contents.json"), JSON.stringify(assetCatalogContents, null, 2));
} catch (error) {
...
if (!_fs.default.existsSync(iconsSourceDir)) {
...
const svgFiles = _fs.default.readdirSync(iconsSourceDir).filter(file => file.toLowerCase().endsWith(".svg")).map(file => _path.default.join(iconsSourceDir, file));
...
(async function processIcons() {
  try {
    for (const svgPath of svgFiles) {
      const iconName = _path.default.basename(svgPath, ".svg");
      ...
      const iconSymbolsetDir = _path.default.join(iosXcassetsDir, `${iosIconName}.symbolset`);
      _fs.default.mkdirSync(iconSymbolsetDir, {
        recursive: true
      });
      ...
      (0, _child_process.execSync)(fixerCommand, {
        stdio: "inherit"
      });
      ...
      const fixedSvgPath = _path.default.join(tempSvgsDir, _path.default.basename(svgPath));
      const androidOutputPath = _path.default.join(androidOutputDir, "ic_" + iconName.replace(/-/g, "_") + ".xml");

      // Check if fixed SVG exists
      if (!_fs.default.existsSync(fixedSvgPath)) {
      ...
      (0, _child_process.execSync)(vectorDrawableCommand, {
        stdio: "inherit"
      });
      ...
      if (_fs.default.existsSync(androidOutputPath)) {
        let xmlContent = _fs.default.readFileSync(androidOutputPath, "utf8");
        ...
        _fs.default.writeFileSync(androidOutputPath, xmlContent);
      ...
      const contentsJson = {
        ...
        symbols: [{
          idiom: "universal",
          filename: _path.default.basename(svgPath)
        }]
      };
      const contentsJsonPath = _path.default.join(iconSymbolsetDir, "Contents.json");
      _fs.default.writeFileSync(contentsJsonPath, JSON.stringify(contentsJson, null, 2));

      // Read the fixed SVG content
      let fixedSvgContent = _fs.default.readFileSync(fixedSvgPath, "utf8");
 ...
 <g id="Symbols">
  ...
  <g id="Ultralight-S" transform="matrix(1 0 0 1 515.484 625.541)">
   <path class="monochrome-0 multicolor-0:systemBlueColor hierarchical-0:primary SFSymbolsPreviewWireframe" d="${extractIconPath(fixedSvgContent, 0)}"/>
      ...
      const iosSvgPath = _path.default.join(iconSymbolsetDir, _path.default.basename(svgPath));
      _fs.default.writeFileSync(iosSvgPath, sfSymbolsSvg);
      console.log(`iOS Symbol files created successfully at: ${iconSymbolsetDir}`);
    }

    // Clean up the temporary SVGs folder
    console.log("\nCleaning up temporary files...");
    if (_fs.default.existsSync(tempSvgsDir)) {
      _fs.default.rmSync(tempSvgsDir, {
        recursive: true,
        force: true
      });
...
function extractSvgPathsData(svgContent) {
  ...
  return paths.length > 0 ? paths.join(" ") : "M24,12 a12,12 0 1,0 -24,0 a12,12 0 1,0 24,0"; // Fallback circle if no paths found
}
...
function extractSvgElements(svgContent) {
  ...
  let content = svgContent.replace(/<svg[^>]*>([\s\S]*)<\/svg>/, "$1").replace(/<g[^>]*>([\s\S]*)<\/g>/g, "$1");
...
function extractIconPath(svgContent, pos) {
  ...
  const transformedPaths = paths.map(path => {
    ...
    return path.replace(/([-]?[0-9]+(\.[0-9]+)?)/g, match => {
      ...
      const transformedNum = isXCoordinate ? num * scaleX + offsetX : num * scaleY + offsetY;
